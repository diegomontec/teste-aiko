...
const MapView = () => {
  ...
  const [selectedEquipmentState, setSelectedEquipmentState] =
    useState<EquipmentState | null>(null);
  const [equipmentStateHistory, setEquipmentStateHistory] =
  ...
  useEffect(() => {
    ...
    addMarkersToMap(
      ...
      (equipment: Equipment | null) => {
        setSelectedEquipment(equipment); // Agora aceita null
        if (equipment) {
          // Verifique se equipment não é null
          ...
          setModalOpen(false); // Fecha a modal se equipment for null
        }
      },
      setModalOpen
    );
